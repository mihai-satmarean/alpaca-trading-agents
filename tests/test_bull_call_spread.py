...
def _make_call(symbol: str, underlying: str, strike: float, dte: int = 30,
               oi: int = 500) -> OptionCandidate:
...
class TestScanFindsValidSpreads:
    def test_two_calls_produce_one_spread(self):
        calls = [
            _make_call("AAPL260101C00098000", "AAPL", 98.0),
            _make_call("AAPL260101C00105000", "AAPL", 105.0),
        ]
    ...
    def test_spread_width_is_correct(self):
        calls = [
            _make_call("X260101C00095000", "X", 95.0),
            _make_call("X260101C00100000", "X", 100.0),
        ]
        s, _ = _strategy(calls=calls, quote_mid=97.0)
        candidates = s.scan(["X"])
        assert len(candidates) >= 1
        assert candidates[0].spread_width == pytest.approx(5.0)

    def test_max_debit_is_per_contract_times_100(self):
        calls = [
            _make_call("X260101C00095000", "X", 95.0),
            _make_call("X260101C00105000", "X", 105.0),
        ]
        s, _ = _strategy(calls=calls, quote_mid=100.0)
        candidates = s.scan(["X"])
        if candidates:
...
class TestScoring:
    def test_higher_reward_risk_scores_better(self):
        calls = [
            _make_call("A260101C00098000", "A", 98.0, dte=25),
            _make_call("A260101C00103000", "A", 103.0, dte=25),
            _make_call("A260101C00110000", "A", 110.0, dte=25),
        ]
        s, _ = _strategy(calls=calls, quote_mid=100.0)
        candidates = s.scan(["A"])
...
class TestExecution:
    def test_spread_order_uses_mleg(self):
        calls = [
            ...
            _make_call("MSFT260101C00410000", "MSFT", 410.0),
        ]
    ...
    def test_spread_order_has_two_legs(self):
        calls = [
            _make_call("V260101C00295000", "V", 295.0),
            _make_call("V260101C00310000", "V", 310.0),
        ]
